...
def test_causal_masking():
    """
    Tests that the causal attention mask is working correctly.
    
    This script verifies that gradients only flow from past/current tokens,
    not from future tokens. If future tokens have gradients, it means the
    causal mask is broken and the model can "see the future" during training.
    """
    model = KhanhLLM()
    model.eval()
    ...
    print("Testing Causal Masking via Gradient Flow...")
    ...
    if abs(grad_at_future) > 1e-5:
        print("FAIL: Gradient flowed from future token! Causal mask is BROKEN.")
    else:
        print("PASS: No gradient from future token. Causal mask is WORKING.")
